...
exports.createBarang = async (req, res) => {
    try {
        ...
        const file = req.files ? req.files.file : null; // Cek apakah req.files ada
        ...
        let fileName = null; // Inisialisasi fileName sebagai null
        if (file) { // Hanya jika file ada, lakukan validasi dan upload
            const fileSize = file.data.length; // Pastikan file.data ada
            const ext = path.extname(file.name).toLowerCase();
            fileName = `${namabarang}-${Date.now()}${ext}`;
        ...
        // Buat barang di database
        const barang = await Barang.create({
            namabarang,
            harga,
            kategoriuuid,
            foto: fileName, // Simpan fileName, bisa null jika tidak ada file
        });
