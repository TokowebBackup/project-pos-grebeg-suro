...
const ProductPerCabang = () => {
  ...
  const handlePayment = async () => {
    ...
    try {
      if (selectedPaymentMethod === "cash") {
        ...
        await processQrisManualPayment(totalOrder);
      }
    } catch (err) {
      console.error(err);
    } finally {
      setIsPaying(false); // ✅ Selesai loading
    }
  };
  ...
  // **Render QR Code**
  const processQrisManualPayment = async (total) => {
    try {
      const response = await axios.post(`${getApiBaseUrl()}/createtransaksicabang`, {
        pembayaran: "qris",
        items: orders.map((order) => ({
      ...
      <Dialog open={receiptDialogOpen} onClose={() => setReceiptDialogOpen(false)}>
        ...
        <DialogContent>
          <div id="receipt-preview" style={{ minWidth: "300px" }}>
            ...
            <Typography align="center">Telephone Pemesan: {customerPhone}</Typography>
            {/* <Typography align="center">Email Pemesan: {customerEmail}</Typography> */}
          ...
          <Grid container spacing={2}>
            {filteredProducts.map((product) => (
              <Grid item xs={6} sm={4} md={3} key={product.uuid}>
                ...
                >
                  <Box sx={{ position: 'relative', pt: '75%', backgroundColor: '#f9f9f9' }}>
                    <CardMedia
                      component="img"
                  ...
                  >
                    {parseInt(product?.stok) === 0 ? "Habis" : "Tambah"}
                  </Button>
                </Card>
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            ...
            startIcon={isPaying ? <CircularProgress size={20} color="inherit" /> : <PrintIcon />}
            disabled={isPaying}
          >
            {isPaying ? 'Processing...' : 'Pay & Print'}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
