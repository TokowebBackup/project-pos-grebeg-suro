...
exports.createTransaksiCabang = async (req, res) => {
    ...
    try {
        ...
        if (pembayaran === 'qris') {
            ...
            console.log("Url QRIS nya : " + qrisUrl);
        ...
        const invoiceHTML = `
            <div style="font-family: monospace; font-size: 14px; max-width: 320px; margin: 0 auto; padding: 10px; border: 1px dashed #333;">
                <h3 style="text-align: center;">INVOICE PEMBELIAN</h3>
                <p style="text-align: center;">Order ID: ${orderId}</p>
                <hr style="border-top: 1px dashed #333;" />

                <p>Customer  : ${customerName || '-'}</p>
                <p>No. Telp  : ${customerPhone || '-'}</p>
                ...
                <tbody>
                    ${validatedItems
                    .map(item => {
                        const nama = barangMap.get(item.baranguuid).namabarang;
                        const total = `Rp${parseInt(item.total).toLocaleString()}`;
                        const harga = `@Rp${parseInt(item.harga).toLocaleString()} x ${item.jumlahbarang}`;
                        return `
                        <tr>
                ...
                <hr style="border-top: 1px dashed #333;" />
                <p><strong>Total Bayar:</strong> Rp${totaljual.toLocaleString()}</p>
                <p><strong>Metode Bayar:</strong> ${pembayaran}</p>
                <hr style="border-top: 1px dashed #333;" />
